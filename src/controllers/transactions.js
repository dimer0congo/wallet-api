...
export async function getTransationsByUserId(req, res) {
   try {
...
export async function deleteTransationsById(req, res) {
   try {
      const { id } = req.params
      ...
      res.status(200).json({
         message: "Transations deleted successfully"
      })
...
export async function getTransactionsSummary(req, res) {
   try {
      ...
      const incomeResult = await sql`
            SELECT COALESCE(SUM(amount),0) as income FROM 
            transactions where user_id = ${userId} AND amount > 0;
        `
